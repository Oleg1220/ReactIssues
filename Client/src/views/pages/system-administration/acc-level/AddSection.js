...
const AddSection = (props) => {
  ...
  const { _state, _onCloseClick,_callback,_accessLevel,_module_id } = props;
  ...
  useEffect(() => {    
    if(tabsState.tabs==1){
      _getRecords(user,setsectionList);      
    }
    return;
  }, [user.token!=""]);
  ...
  function handleSelectChange(e){
    if(e){
      console.log(e)
      if(e.value!=""){
        updateForm({ section_id: e.value,section_name:e.label })
      }
  ...
  async function onSubmit(e) {
    e.preventDefault();
    setAlertState(null);
    form.section_id = form.section_id.trim();
    console.log(form);
    ...
    const _payload = {
      payload: { ...form, access_level:_accessLevel,module_id:_module_id, added_by:user.fullname, date_added: new Date()},
    ...
    _dataService.CRUD(window.base_api+`al_sections/add`,requestOptions, async (response)=>{
  ...
  async function _getRecords(user,callback){
    ...
    if(token && token_creation){
      const _payload = { 
        ...
        payload:{access_level:_accessLevel,module_id:_module_id} 
      };
      ...
      _dataService.CRUD(window.base_api+`get_al_sectionsDropdown/`,requestOptions, async (response)=>{
