...
const AddAction = (props) => {
  ...
  const { _state, _onCloseClick,_callback,_accessLevel,_module_id, _section_id} = props;
  ...
  const [actionList, setActionList] = useState( [{
    value: "",
    label: "All sections have been added",
    isDisabled:true
  }]);
  ...
  useEffect(() => {    
    if(tabsState.tabs==1){
      _getRecords(user,setActionList);      
    }
    return;
  }, [user.token!=""]);
  ...
  function handleSelectChange(e){
    if(e){
      console.log(e)
      if(e.value!=""){
        updateForm({ action_id: e.value,action_name:e.label })
      }
       
    }
    else{
        updateForm({ action_id: "" })
    }
  ...
  async function onSubmit(e) {
    e.preventDefault();
    setAlertState(null);
    form.action_id = form.action_id.trim();
    console.log(form);
    ...
    const _payload = {
      payload: { ...form, access_level:_accessLevel,module_id:_module_id,section_id:_section_id, added_by:user.fullname, date_added: new Date()},
    ...
    _dataService.CRUD(window.base_api+`al_actions/add`,requestOptions, async (response)=>{
  ...
  async function _getRecords(user,callback){
    ...
    if(token && token_creation){
      const _payload = { 
        ...
        payload:{access_level:_accessLevel,module_id:_module_id,section_id:_section_id} 
      };
      ...
      _dataService.CRUD(window.base_api+`get_al_actionsDropdown/`,requestOptions, async (response)=>{
